...
def build_oauth_router(auth_service: AuthService) -> APIRouter:
    ...
    def oauth_callback(
        ...
        try:
            profile = exchange_code_for_profile(provider, code, stored["redirect_uri"])
        ...
        # Hand tokens to frontend via fragment-like query (dev MVP).
        q = f"token={token}&refreshToken={refresh}&oauth={provider}"
        return RedirectResponse(f"{settings.app_public_url}/login?{q}")
