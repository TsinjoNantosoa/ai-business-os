...
import {
  ...
  TrendingUp, Wallet, Calendar, Zap, BookOpen,
} from 'lucide-react';
    ...
    <div className="mt-2 flex flex-wrap gap-1.5">
      {sources.map((s) => (
        ...
        >
          ...
          <span className="text-muted-foreground">{Math.round((s.relevanceScore || 0) * 100)}%</span>
        </Badge>
      ))}
    </div>
  );
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={cn('flex gap-2.5', msg.role === 'user' && 'flex-row-reverse')}>
                  ...
                  >
                    ...
                    {msg.role === 'assistant' && !msg.streaming && msg.sources && <SourceChips sources={msg.sources} />}
